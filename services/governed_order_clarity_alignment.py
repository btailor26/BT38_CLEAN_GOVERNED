...
def _clean_fbm_journey_html(html: str) -> str:
    value = str(html or "")
    for old,new in _JOURNEY_LABEL_REPLACEMENTS: value=value.replace(old,new)
    return value

def _inject_once(html: str, marker: str, payload: str, closing: str) -> str:
    value=str(html or "")
    if marker in value: return value
...
def install_governed_order_clarity_alignment(app) -> None:
    if getattr(app,"_bt38_order_clarity_alignment_installed",False): return
