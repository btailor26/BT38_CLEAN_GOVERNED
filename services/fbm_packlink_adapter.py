...
class PacklinkAdapter:
    ...
    def create_shipment_draft(self, *, order: Any, parcel: dict[str, Any], rate: dict[str, Any]) -> dict[str, Any]:
        """Create a Packlink draft with the same address facts as a Packlink import.

        The canonical address remains top-level ``from``/``to``. Packlink's inbound
        draft model also hydrates the editable PRO form from ``additional_data``;
        therefore the same values are mirrored there byte-for-byte. No frontend
        postal-zone IDs, postcode IDs, warehouse IDs or synthetic parcel IDs are
        generated by BT38.
        """
