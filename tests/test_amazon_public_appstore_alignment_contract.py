...
LOGO_REF = "img/marketplaces/bt38-inventory-amazon-developer.svg"
...
def test_all_public_surfaces_use_the_same_canonical_brand_asset():
    ...
    for path in (LANDING, PRIVACY, TERMS, SUPPORT, APPLY, RECEIVED):
        assert LOGO_REF in _text(path), f"canonical logo missing from public surface {path}"
...
def test_public_policy_support_and_application_destinations_are_real_routes():
    ...
    assert '@app.route("/apply", methods=["GET", "POST"])' in routes
...
def test_landing_contains_amazon_functionality_claims_selected_for_submission():
    text = _text(LANDING)
    required = (
    ...
    for marker in required:
        assert marker in text, f"Amazon-facing functionality marker missing: {marker}"
