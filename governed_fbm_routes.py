...
def _marketplace_shipping_mode(order: MarketplaceOrder, platform: str) -> dict:
    ...
    if normalized == "amazon":
        return {"recommended": "Amazon Buy Shipping", "marketplace_buy_shipping": True, "external_provider": True, "manual": True, "reason": "Amazon-native rates/labels are first-class; SFP/Prime eligibility must come from Amazon, not BT38."}
    if normalized == "ebay":
        return {"recommended": "Best connected provider", "marketplace_buy_shipping": True, "external_provider": True, "manual": True, "reason": "Show eBay-native shipping when the connected account/API exposes it; otherwise Packlink/direct carrier remains available."}
    return {"recommended": "Best connected provider", "marketplace_buy_shipping": False, "external_provider": True, "manual": True, "reason": "Provider availability is determined by the seller's connected accounts."}
...
def _shipping_provider_options(order: MarketplaceOrder) -> list[dict]:
    ...
    if platform == "amazon":
        ...
        options.append({"provider": "amazon_buy_shipping", "label": "Amazon Buy Shipping", "kind": "marketplace", "configured": configured, "available": configured, "recommended": True, "supports_prime_sfp": True, "label_formats": ["PDF", "PNG", "ZPL"], "auto_print_supported": True, "requires_terms_acceptance": True, "message": "Amazon returns eligible services and label formats for the order. Auto-print can be used after purchase when QZ Tray is connected and a printer preference is saved." if configured else "Amazon credentials are not available for this store."})
    if platform == "ebay":
        creds = getattr(store, "ebay_credentials", None) if store is not None else None
        configured = bool(creds and getattr(creds, "is_valid", lambda: False)())
        options.append({"provider": "ebay_shipping", "label": "eBay Shipping", "kind": "marketplace", "configured": configured, "available": False, "recommended": False, "supports_prime_sfp": False, "label_formats": ["PDF"], "auto_print_supported": True, "requires_terms_acceptance": False, "message": "eBay account is connected. Native label buying enables only when the app/account exposes a supported UK shipping-label API." if configured else "eBay credentials are not available for this store."})
    packlink = PacklinkAdapter()
    options.append({"provider": "packlink", "label": "Packlink PRO", "kind": "provider", "configured": packlink.configured, "available": packlink.configured, "recommended": platform != "amazon", "supports_prime_sfp": False, "label_formats": ["PROVIDER"], "auto_print_supported": True, "requires_terms_acceptance": False, "message": "Packlink PRO is connected. BT38 will preserve the label returned by the selected carrier and can send it to the saved printer after purchase." if packlink.configured else "PACKLINK_API_KEY is not configured."})
    options.append({"provider": "manual", "label": "Manual / own carrier", "kind": "manual", "configured": True, "available": True, "recommended": False, "supports_prime_sfp": False, "label_formats": [], "auto_print_supported": False, "requires_terms_acceptance": False, "message": "Use a label bought outside BT38. BT38 will still normalise carrier, service and tracking before marketplace confirmation."})
    return options
...
def fbm_page():
    ...
    for row in rows:
        ...
        orders.append({"order": row, "platform": platform, "store_name": _store_name(row), "route_state": route_state, "shipping_mode": _marketplace_shipping_mode(row, platform), "shipment": shipment, "shipment_state": shipment_state, "case": case})
...
def fbm_shipping_options():
    ...
    for order_id in order_ids:
        ...
        if row is None:
            continue
        result.append({"id": row.id, "marketplace_order_id": row.marketplace_order_id, "platform": _platform(row), "store_name": _store_name(row), "sku": getattr(row, "sku", None), "quantity": getattr(row, "quantity", None), "postcode": getattr(row, "ship_to_postcode", None), "route_state": _route_state(row), "providers": _shipping_provider_options(row)})
    if not result:
        return jsonify({"success": False, "message": "No selected orders are eligible for FBM shipping."}), 404
    return jsonify({"success": True, "read_only": True, "orders": result, "selected_count": len(result), "printing": {"mode": "qz_tray", "auto_print_after_purchase": True, "printer_preference_required": True, "fallback": "download_label"}, "message": "Shipping routes prepared. Purchase and automatic printing remain confirmation-gated."})
